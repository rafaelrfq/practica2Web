        ...
        app.routes(() -> {
            ...
            path("/api/", () -> {
                ...
                post("/productos/crud/editar/:id", ctx -> {
                    Producto producto = tienda.getProductoPorID(ctx.pathParam("id", Integer.class).get());
                    String nombre = ctx.formParam("nombre");
                    BigDecimal precio = new BigDecimal(ctx.formParam("precio"));
                    producto.actualizar(new Producto(producto.getId(), nombre, precio));
                    ctx.redirect("/api/productos/crud");
                });
                ...
                post("/carrito/procesar/", ctx -> {
                    ...
                    VentasProducto venta = new VentasProducto(id, new Date(), nombreCliente, tienda.getCarrito().getListaProductos());
